...
return m.reply('❤️‍ Si espacios para nuevos subbots yue.')
}
let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender
let id = `${who.split`@`[0]}`  //conn.getName(who)
let pathYueJadiBot = path.join(`./${jadi}/`, id)
if (!fs.existsSync(pathYueJadiBot)){
fs.mkdirSync(pathYueJadiBot, { recursive: true })
}
yueJBOptions.pathYueJadiBot = pathYueJadiBot
yueJBOptions.m = m
yueJBOptions.conn = conn
yueJBOptions.args = args
yueJBOptions.usedPrefix = usedPrefix
yueJBOptions.command = command
yueJadiBot(yueJBOptions)
...
export async function yueJadiBot(options) {
let { pathYueJadiBot, m, conn, args, usedPrefix, command } = options
...
const pathCreds = path.join(pathYueJadiBot, "creds.json")
if (!fs.existsSync(pathYueJadiBot)){
fs.mkdirSync(pathYueJadiBot, { recursive: true })}
...
const { state, saveState, saveCreds } = await useMultiFileAuthState(pathYueJadiBot)
...
browser: mcode ? ['Ubuntu', 'Chrome', '110.0.5585.95'] : ['Yue-Bot (Sub Bot)', 'Chrome','2.0.0'],
...
const nameOrNumber = conn.getName(`${path.basename(pathYueJadiBot)}@s.whatsapp.net`)
const baseName = path.basename(pathYueJadiBot)
